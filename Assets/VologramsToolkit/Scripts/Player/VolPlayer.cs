...
public class VolPlayer : MonoBehaviour
{
    ...
    string wwwDownfile(string filePath, bool isWebURL)
    {
        string[] urlID = filePath.Split('/');
        string folderp;
        string webreqURL = filePath;
        if (volFolderPathType == VolEnums.PathType.Streaming) //url
        {
            webreqURL = (Path.Combine("jar:file://" + Application.dataPath + "!/assets", filePath));
        }
        if (urlID.Length >= 2) folderp = Path.Combine(Application.persistentDataPath, urlID[urlID.Length - 2]);
        else
        {
            folderp = Path.Combine(Application.persistentDataPath, urlID[0]);
        }
        ...
        var loadingRequest = UnityWebRequest.Get(webreqURL);
        ...
        {
            Debug.LogWarning("resultPath: " + resultPath + "; " + loadingRequest.result.ToString() + "; " + loadingRequest.uri.ToString());
            // File.Create(resultPath);
            File.WriteAllBytes(resultPath, loadingRequest.downloadHandler.data);
            return resultPath;
        }
    }
    /// <summary>
    ...
    public bool Open()
    {
        ...
        if (_hasVideoTexture)
        {
            if (volFolderPathType == VolEnums.PathType.Absolute)
            {
                _fullVideoPath = wwwDownfile(volVideoTexture, true);
            }
        ...
        /* if (Application.platform == RuntimePlatform.Android)
         {
             Debug.Log("Writing geom file");
             headerFile = wwwDownfile(volFolder+ "/header.vols");
             sequenceFile = wwwDownfile(volFolder+ "/sequence_0.vols");
...
#if UNITY_EDITOR
            _meshRenderer.sharedMaterial.SetTexture(_textureId, _voloTexture);
            //Texture2D copyTexture  = new Texture2D( VolPluginInterface.VolGetVideoWidth(), VolPluginInterface.VolGetVideoHeight(), TextureFormat.RGB24, false, false);
            //copyTexture.LoadRawTextureData(_voloTexture.GetRawTextureData());
            //copyTexture.Apply();
            //copyTexture.SetPixels(_voloTexture.GetPixels());
            //copyTexture.Apply(); 
    ...
    private void ReadGeomFrame(int frame)
    {
        ...
        //Mesh newMesh = (Mesh)Instantiate(_meshFilter.sharedMesh);
